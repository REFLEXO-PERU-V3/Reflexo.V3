...
const EditAppointment = ({ appointmentId, onEditSuccess }) => {
  ...
  const [isLightTheme, setIsLightTheme] = useState(() => {
    try {
      const saved = localStorage.getItem('editAppointmentTheme');
  ...
  useEffect(() => {
    try {
      localStorage.setItem('editAppointmentTheme', isLightTheme ? 'light' : 'dark');
    } catch {}
  }, [isLightTheme]);
  ...
  // Si no hay appointmentId, permitir mostrar el formulario vacío
  useEffect(() => {
    if (!appointmentId) {
  ...
  return (
    <div className={`${styles.page} ${isLightTheme ? styles.themeLight : styles.themeDark}`}>
      <div className={styles.wrapper}>
        <Title level={3} className={styles.title}>
          Editar Cita
        </Title>
        <Divider className={styles.accentDivider} />
        <div className={styles.themeToggleRow}>
          <Space>
            <span className={styles.themeLabel}>Tema claro</span>
            <Switch checked={isLightTheme} onChange={setIsLightTheme} />
          </Space>
        </div>
          <Form
            form={form}
          ...
          >
            ...
            <Row gutter={16}>
              ...
              <Col span={8}>
                <Space className={styles.checkboxTop}>
                  <Button
                    type="primary"
          ...
          <Modal
            ...
            rootClassName={`${styles.modalRoot} ${isLightTheme ? styles.themeLight : styles.themeDark}`}
            centered
            onCancel={() => setIsSelectPatientModalOpen(false)}
            footer={[
              <Button key="cancel" onClick={() => setIsSelectPatientModalOpen(false)}>
                Cancelar
              </Button>,
              <Button key="select" type="primary" onClick={handlePatientSelection}>
                Seleccionar
              </Button>,
            ]}
            width={500}
          ...
          <Modal
            ...
            destroyOnHidden
          >
            <NewPatient onCancel={() => setIsCreatePatientModalOpen(false)} onSubmit={handlePatientCreated} />
          </Modal>
        </div>
      </div>
  );
