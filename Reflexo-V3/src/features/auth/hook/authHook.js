...
export const useAuth = () => {
  ...
  const login = async (credentials) => {
    setLoading(true);
    try {
      // Mock login: bypass API when credentials are admin/admin
      if (
        (credentials.email === 'admin@gmail.com' || credentials.username === 'admin') &&
        credentials.password === 'admin'
      ) {
        // Persist mock session
        persistLocalStorage('token', 'mock-token');
        persistLocalStorage('user_role', 1); // admin
        persistLocalStorage('name', 'Administrador');
